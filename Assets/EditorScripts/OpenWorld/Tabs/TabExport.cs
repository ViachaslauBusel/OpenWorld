    ...
    public class TabExport
    {
        ...
        public static void Draw()
        {
                ...
                if (GUILayout.Button("build asset bundles"))
            {
                ...
                Map map = TabSetting.Map;
                string mapName = map.Name.ToLower();
                ...
                float maxProgress = map.Size * map.Size + map.TilesPerBlock * map.TilesPerBlock;
                for (int yKM = 0; yKM < map.Size; yKM++)
                {
                    for (int xKM = 0; xKM < map.Size; xKM++)
                    {
                        for (int yTR = 0; yTR < map.TilesPerBlock; yTR++)
                        {
                            for (int xTR = 0; xTR < map.TilesPerBlock; xTR++)
                            {

                                EditorUtility.DisplayProgressBar("OpenWorld", "attach objects", ((yKM * map.Size + xKM) + (yTR * map.TilesPerBlock + xTR)) / maxProgress);
            ...
            {
                float maxProgress = map.Size * map.Size;
                ...
                for (int x = 0; x < map.Size; x++)
                {
                    ...
                    for (int y = 0; y < map.Size; y++)
                    {
                        ...
                        folder = map.Name + "/KMObject_" + x + '_' + y;
            ...
            if (GUILayout.Button("Remove missing Tree/Grass"))
            {
                ...
                for (int x = 0; x < map.Size; x++)
                {
                    ...
                    for (int y = 0; y < map.Size; y++)
                    {
                        ...
                        folder = map.Name + "/KMBlock_" + x + '_' + y;
                    //    Remove(folder);
                        Fix(folder);
                    }
                }
